...
def draw_ep(el=Dia, t=500, absolute=False, mass=False, logy=False, y_range=None, xmin=.1, xmax=1e3):
    bethe_el = BetheBloch(Electron, el, t, absolute=absolute, mass=mass).draw(y_range=y_range, xmin=xmin, xmax=xmax, logy=logy, color=Draw.get_colors(10)[0])
    bethe_pos = BetheBloch(Positron, el, t, absolute=absolute, mass=mass).draw_same(Draw.get_colors(10)[0], 2)
    brems = Bremsstrahlung(Electron, el, t, absolute, mass).draw_same(Draw.get_colors(10)[7], 10)
    data = el.draw_brems(not absolute, mass, t=t / 10, color=Draw.get_colors(10)[9])
...
def get_losses(p, el, t=500):
    return array([BetheBloch(part, el, t, absolute=True)(p) for part in [Pion, Muon, Positron, Proton]])
...
def print_energies(p, el: Element, t=500, table=False):
    ...
    for part in particles:
        e = BetheBloch(part, el, t, absolute=True)(p)
